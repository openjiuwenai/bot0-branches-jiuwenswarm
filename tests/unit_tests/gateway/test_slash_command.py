...
def test_web_slash_picker_command_contract() -> None:
    ...
    assert [command["name"] for command in commands] == ["new", "fork", "compact", "plan", "persist"]
    ...
    assert commands[2]["req_method"] == "command.compact"
    assert commands[3]["execution"] == "chat.send_with_mode"
    assert commands[3]["takesArgs"] is False
    assert commands[3]["usage"] == "/plan"
    assert commands[3]["example"] is None
    assert commands[3]["plan_entry_source"] == "slash_command"
